...
struct WIN32_EXPORT Socket : Poll {
 protected:
  ...
  struct Queue {
    struct Message {
      ...
      void (*callback)(void *socket, void *data, bool cancelled,
  ...
  void setShuttingDown(bool shuttingDown) { state.shuttingDown = shuttingDown; }

  void transfer(NodeData *nodeData, void (*cb)(Poll *)) {
    // userData is invalid from now on till onTransfer
    setUserData(new TransferData(
        {getFd(), ssl, getCb(), getPoll(), getUserData(), nodeData, cb}));
  ...
  static void sslIoHandler(Poll *p, int status, int events) {
    ...
    if (!socket->messageQueue.empty() &&
        ((events & UV_WRITABLE) || SSL_want(socket->ssl) == SSL_READING)) {
      socket->cork(true);
      while (true) {
        Queue::Message *messagePtr = socket->messageQueue.front();
        int sent =
            SSL_write(socket->ssl, messagePtr->data, (int)messagePtr->length);
        if (sent == (ssize_t)messagePtr->length) {
          ...
          if (socket->messageQueue.empty()) {
            if ((socket->state.poll & UV_WRITABLE) &&
                SSL_want(socket->ssl) != SSL_WRITING) {
          ...
          switch (SSL_get_error(socket->ssl, sent)) {
            ...
            case SSL_ERROR_WANT_WRITE:
              if ((socket->getPoll() & UV_WRITABLE) == 0) {
                socket->change(
                    socket->nodeData->loop, socket,
                    socket->setPoll(socket->getPoll() | UV_WRITABLE));
              }
              break;
            default:
              STATE::onEnd((Socket *)p);
    ...
    if (events & UV_READABLE) {
      do {
        int length = SSL_read(socket->ssl, socket->nodeData->recvBuffer,
                              socket->nodeData->recvLength);
        if (length <= 0) {
          ...
          // Warning: onData can delete the socket! Happens when HttpSocket
          // upgrades
          socket =
              STATE::onData((Socket *)p, socket->nodeData->recvBuffer, length);
    ...
    if (events & UV_READABLE) {
      ...
      if (length > 0) {
        STATE::onData((Socket *)p, nodeData->recvBuffer, length);
                 ...
                 (length == SOCKET_ERROR && !netContext->wouldBlock())) {
        STATE::onEnd((Socket *)p);
      }
    }
  }
  ...
  bool hasEmptyQueue() { return messageQueue.empty(); }

  void enqueue(Queue::Message *message) { messageQueue.push(message); }

  Queue::Message *allocMessage(size_t length, const char *data = 0) {
    Queue::Message *messagePtr =
        (Queue::Message *)new char[sizeof(Queue::Message) + length];
  ...
  void freeMessage(Queue::Message *message) { delete[](char *) message; }
  ...
  void sendTransformed(const char *message, size_t length,
                       void (*callback)(void *socket, void *data,
                                        bool cancelled, void *reserved),
                       void *callbackData, D transformData) {
    size_t estimatedLength =
        T::estimate(message, length) + sizeof(Queue::Message);
    ...
    if (hasEmptyQueue()) {
      if (estimatedLength <= uS::NodeData::preAllocMaxSize) {
        ...
        Queue::Message *messagePtr =
            (Queue::Message *)nodeData->getSmallMemoryBlock(memoryIndex);
        messagePtr->data = ((char *)messagePtr) + sizeof(Queue::Message);
        messagePtr->length = T::transform(message, (char *)messagePtr->data,
                                          length, transformData);
        ...
        Queue::Message *messagePtr =
            allocMessage(estimatedLength - sizeof(Queue::Message));
        messagePtr->length = T::transform(message, (char *)messagePtr->data,
                                          length, transformData);
 ...
 public:
  ...
  Socket(NodeData *nodeData, Loop *loop, uv_os_sock_t fd, SSL *ssl)
      : Poll(loop, fd), ssl(ssl), nodeData(nodeData) {
    if (ssl) {
  ...
  void setServerId(std::string id) { serverId = id; }

  void setUserData(void *user) { this->user = user; }
  ...
  void closeSocket() {
    ...
    Poll::close(nodeData->loop, [](Poll *p) { delete (T *)p; });
  }

  bool isShuttingDown() { return state.shuttingDown; }
...
struct ListenSocket : Socket {
  ListenSocket(NodeData *nodeData, Loop *loop, uv_os_sock_t fd, SSL *ssl)
      : Socket(nodeData, loop, fd, ssl) {}
